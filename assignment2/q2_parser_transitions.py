...
def minibatch_parse(sentences, model, batch_size):
    ...
    ### YOUR CODE HERE
    # had to consult https://github.com/zysalice/cs224/blob/master/assignment2/q2_parser_transitions.py#L79
    ...
    while len(unfinished_parse) > 0:
        ...
        while (len(minibatch) > 0):
            ...
            for n in range(len(transitions) - 1, -1, -1):
                minibatch[n].parse_step(transitions[n])
                if len(minibatch[n].stack) < 2:
                    # delete the parser from the minibatch if it is completed
                    del minibatch[n]
